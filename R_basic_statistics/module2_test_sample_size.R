...
##############################
##Part 2
## This is an example from workshop in basic statistics in R held in 2020 by Meena Choi (one of the co-creaters of MSStats
...
##is there a significant difference between the means of two groups?- are they coming from the same population?
...
# log2 transformed fold change between groups : mean for Condition1 - mean for Condition2
result$estimate[1]-result$estimate[2]

# test statistic value, 
# T value - the difference represented in units of standard error. 
...
# degree of freedom
#determined by the number of observations in your sample
#the number of independent pieces of information that went into calculating the estimate:n – 1
result$parameter 
...
## Volcano plot
## shows the relationship between the p-values of a statistical test and the magnitude of the difference in expression values of the samples in the groups. 
##On the y-axis the -log10 p-values are plotted. 
##For the x-axis you may choose between two sets of values by choosing either 'Fold change' or 'Difference'
##The larger the difference in expression of a feature, the more extreme it's point will lie on the X-axis. 
##The more significant the difference, the smaller the p-value and thus the higher the -log10(p) value. 
##Thus, points for features with highly significant differences will lie high in the plot. 
##Features of interest are typically those which change significantly and by a certain magnitude. 
##These are the points in the upper left and upper right hand parts of the volcano plot.
allresult %>%
